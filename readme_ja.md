...
======================
**スクリプトをダウンロードするには、このページの右上あたりにある Clone or Download ボタンを押し、Download ZIP を選んでください。ファイル一覧の右クリックから「別名で保存」を実行して保存されるのは、リンク先のソース(HTMLファイル)です。**
...
なぜならキャンセルボタンがあるので途中で止めることができるからです。また、コンソールパレットで誤差が収束する様子も見ることができます。**条件が悪いと、Illustrator を強制終了するしか止める方法がなくなる恐れがあります**。
